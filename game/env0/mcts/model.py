...
class Gen_Model():
	...
	def printWeightAverages(self):
		...
		for i, l in enumerate(layers):
			try:
				x = l.get_weights()[0]
				printf('WEIGHT LAYER %d: ABSAV = %f, SD =%f, ABSMAX =%f, ABSMIN =%f', i, np.mean(np.abs(x)), np.std(x), np.max(np.abs(x)), np.min(np.abs(x)))
			except:
				pass
		print(''.zfill(10))
		for i, l in enumerate(layers):
...
class Residual_CNN(Gen_Model):
	...
	def _build_model(self):
		...
		model.compile(loss={'value_head': 'mean_squared_error', 'policy_head': softmax_cross_entropy_with_logits},
			optimizer=SGD(lr=self.learning_rate, momentum = mcts_config.momentum),	
